...
def zpFFTsizeExpt(x, fs):
    ...
    ## Your code here 
    outputScaleFactor1 = sum(w1)
    outputScaleFactor2 = sum(w2)
    ...
    plt.subplot(3,1,1)
    plt.plot(float(fs)*np.arange(mX1.size)/float(M),mX1, color="red")
    plt.axis([0, fs/2.0, min(mX1), max(mX1)])
    plt.title ('magnitude spectrum: mX1')
    plt.ylabel('dB')
    plt.xlabel('frequency (Hz)')
    
    
    plt.subplot(3,1,2)
    plt.plot(float(fs)*np.arange(mX2.size)/float(2*M),mX2, color="blue")
    plt.axis([0, fs/2.0, min(mX2), max(mX2)])
    plt.title ('magnitude spectrum: mX2')
    plt.ylabel('dB')
    plt.xlabel('frequency (Hz)')
    
    plt.subplot(3,1,3)
    plt.plot(float(fs)*np.arange(mX3.size)/float(2*M),mX3,color="green")
    plt.axis([0, fs/2.0, min(mX3), max(mX3)])
    plt.title ('magnitude spectrum: mX3')
    plt.ylabel('dB')
    plt.xlabel('frequency (Hz)')
